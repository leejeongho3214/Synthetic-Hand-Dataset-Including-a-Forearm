...
def main(args):
    ...
    for epoch in range(epo, args.epoch):
        if epoch == epo: 
            print(colored(f"Path: {args.output_dir} | Dataset: {args.dataset} | Train_len: {len(train_dataset)} | Test_len: {len(test_dataset)} |  Status: {args.reset} | add_our: {(args.ratio_of_our * 100):.0f}% | align: {args.set} | 2D_loss: {args.loss_2d} | 3D_loss: {args.loss_3d} | 3D_mid_loss: {args.loss_3d_mid} | noise_c: {(args.ratio_of_aug * 100):.0f}% | Max_count : {args.count} | Max_epoch : {args.epoch}", color = "yellow"))
            args.logger.debug(f"Path: {args.output_dir} | Dataset: {args.dataset} | Train_len: {len(train_dataset)} | Test_len: {len(test_dataset)} | Status: {args.reset} | add_our: {(args.ratio_of_our * 100):.0f}% | align: {args.set} | 2D_loss: {args.loss_2d} | 3D_loss: {args.loss_3d} | 3D_mid_loss: {args.loss_3d_mid} | noise_c: {(args.ratio_of_aug * 100):.0f}% | Max_count : {args.count} | Max_epoch : {args.epoch}")
