...
def visualize_prediction(images, pred_2d_joint, fig, method = None, epoch = 0, iteration = 0, args =None, dataset_name = 'p'):
    # num = 0
    # images = images[None, :, :, :]
    # pred_2d_joint = pred_2d_joint[None, :, :]
    ...
    if method == 'evaluation':
        ...
        if os.path.isdir(f"eval_image") == False:
            mkdir("eval_image")
        if os.path.isdir(f'eval_image/{args.name[13:-31]}') == False:
            mkdir(f'eval_image/{args.name[13:-31]}')
...
def visualize_prediction_video(images, pred_2d_joint):
    num = 0
